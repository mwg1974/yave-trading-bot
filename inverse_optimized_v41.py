...
# =============================================================================
# INVERSE PRICE ACTION — V4.1 (PnL TRACKING FIX)
# =============================================================================
# Fix: Reset ALLE variabelen na exit, zodat notify_trade() correct werkt
# =============================================================================
...
class InverseOptimizedV41Strategy(bt.Strategy):
    params = (
        ('lookback', 50),
        ('stop_loss_pips', 40),
        ('take_profit_pips', 80),
    ...
    def notify_order(self, order):
        if order.status in [order.Completed]:
            ...
            # ✅ FIX: Reset order pas NA executie
            self.order = None
        elif order.status in [order.Canceled, order.Margin, order.Rejected]:
            self.order = None
    
    def notify_trade(self, trade):
        # ✅ FIX: Alleen closed trades tellen
    ...
    def _reset_position(self):
        """✅ FIX: Reset ALLE positie variabelen na exit."""
        self.in_position = False
        self.entry_price = 0
        self.position_type = ''
        self.sl_price = 0
    ...
    def next(self):
        # ✅ FIX: Check EERST of we in positie zitten
        if self.in_position:
        ...
        # ✅ FIX: Alleen entry als we NIET in positie zitten EN geen open order
        if self.order:
            return
    ...
    def stop(self):
        ...
        print(f"\n{'='*70}")
        print(f"📊 INVERSE OPTIMIZED V4.1 RESULTS")
        print(f"{'='*70}")
...
if __name__ == "__main__":
    print("="*70)
    print("🚀 INVERSE PRICE ACTION — V4.1 (PnL TRACKING FIX)")
    print("="*70)
    
    cerebro = bt.Cerebro(runonce=False, preload=False, exactbars=1)
    
    cerebro.addstrategy(
        InverseOptimizedV41Strategy,
        lookback=50,
        stop_loss_pips=40,
        take_profit_pips=80,
        max_candles=25,
    ...
    df.to_csv("eurusd_m15_inverse_v41.csv")
    print(f"✅ {len(df)} M15 candles")
    
    data = bt.feeds.GenericCSVData(
        dataname="eurusd_m15_inverse_v41.csv",
