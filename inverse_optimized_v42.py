# =============================================================================
# INVERSE PRICE ACTION — OPTIMIZED V4.2 (SWEET SPOT TWEAKS)
# =============================================================================
# Kleine verbeteringen t.o.v. V4.1:
# - Max 28 candles (i.p.v. 25) — iets meer tijd
# - Trail activeert bij 45 pips (i.p.v. 40) — iets later
# - TP 90 pips (i.p.v. 80) — dichterbij 1:2.25 RR
# - Trail distance 22 pips (i.p.v. 20) — iets meer ruimte
# =============================================================================
...
class InverseOptimizedV42Strategy(bt.Strategy):
    params = (
        ('lookback', 50),
        ('stop_loss_pips', 40),       # 40 pips SL (ongewijzigd)
        ('take_profit_pips', 90),     # 90 pips TP (i.p.v. 80)
        ('max_candles', 28),          # 28 candles (i.p.v. 25)
        ('trail_activation_pips', 45), # Start trail bij 45 pips (i.p.v. 40)
        ('trail_distance_pips', 22),   # Trail afstand 22 pips (i.p.v. 20)
    )
    ...
    def next(self):
        ...
        if current_price > resistance:
            ...
            self.log(f'   TP: {self.tp_price:.5f} ({self.params.take_profit_pips} pips) | 1:{self.params.take_profit_pips/self.params.stop_loss_pips:.2f} RR')
    ...
    def stop(self):
        ...
        print(f"\n{'='*70}")
        print(f"📊 INVERSE OPTIMIZED V4.2 RESULTS")
        print(f"{'='*70}")
        ...
        if total_closed > 0:
            ...
            if self.win_count > 0 and self.loss_count > 0:
                ...
                if avg_loss > 0:
                    ...
                    print(f"   Target RR: 1:{self.params.take_profit_pips/self.params.stop_loss_pips:.2f}")
                
                pf = (self.win_count * avg_win) / (self.loss_count * avg_loss)
                print(f"   Profit Factor: {pf:.2f}")
                
                # Vergelijk met V4.1
                print(f"\n📊 Vergelijking met V4.1:")
                print(f"   V4.1 PnL: +$1,798 | V4.2 PnL: ${self.total_pnl:.2f}")
                print(f"   V4.1 Return: 15.09% | V4.2 Return: {((self.total_pnl/10000))*100:.2f}%")
                
                if self.total_pnl > 1798:
                    print(f"   🏆 V4.2 is BETER dan V4.1!")
                elif self.total_pnl > 1500:
                    print(f"   ✅ V4.2 is vergelijkbaar met V4.1")
                else:
                    print(f"   ⚠️  V4.1 is beter — gebruik V4.1 voor forward test")
        print(f"{'='*70}")
...
if __name__ == "__main__":
    print("="*70)
    print("🚀 INVERSE PRICE ACTION — V4.2 (SWEET SPOT TWEAKS)")
    print("="*70)
    print("\n📋 V4.2 vs V4.1 Veranderingen:")
    print("   Max Candles: 25 → 28")
    print("   Trail Activation: 40 → 45 pips")
    print("   Take Profit: 80 → 90 pips")
    print("   Trail Distance: 20 → 22 pips")
    print("="*70)
    
    cerebro = bt.Cerebro(runonce=False, preload=False, exactbars=1)
    
    cerebro.addstrategy(
        InverseOptimizedV42Strategy,
        lookback=50,
        stop_loss_pips=40,
        take_profit_pips=90,       # 90 pips TP
        max_candles=28,            # 28 candles
        trail_activation_pips=45,  # Start trail bij 45 pips
        trail_distance_pips=22,    # 22 pips trail
    )
    ...
    df.to_csv("eurusd_m15_inverse_v42.csv")
    print(f"✅ {len(df)} M15 candles")
    
    data = bt.feeds.GenericCSVData(
        dataname="eurusd_m15_inverse_v42.csv",
